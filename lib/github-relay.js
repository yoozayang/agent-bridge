...
const TYPES = new Set(["bridge_ping", "project_git_status", "azure_work_item_read",
  "azure_work_item_update", "codex_dispatch", "project_text_search", "project_file_read", "project_comment_replace", "project_git_diff"]);
const MAX_PAYLOAD = 16 * 1024;
const MAX_FILE = 1024 * 1024;
const AZURE_PROJECT = "IoTMart 3.0";
const CODEX_MODES = { read_only: "read-only", workspace_write: "workspace-write" };
...
function gitRaw(root, args) {
  ...
  if (run.error) throw run.error;
  if (run.status !== 0) throw new Error(clip(run.stderr || run.stdout || `git exited ${run.status}`, 2000));
  return run.stdout || "";
}
...
function multilineText(value, name, max) {
...
function validate(file, task) {
  ...
  const allowed = {
    ...
    azure_work_item_update: ["id", "type", "work_item_id", "title", "description", "acceptance_criteria", "created_at"],
    codex_dispatch: ["id", "type", "project", "instruction", "mode", "timeout_seconds", "created_at"],
  ...
  if (task.type === "azure_work_item_update") {
    if (!Number.isSafeInteger(task.work_item_id) || task.work_item_id < 1) throw new Error("work_item_id must be a positive integer");
    if (task.title != null) text(task.title, "title", 512);
    if (task.description != null) multilineText(task.description, "description", 12000);
    if (task.acceptance_criteria != null) multilineText(task.acceptance_criteria, "acceptance_criteria", 12000);
    if (task.title == null && task.description == null && task.acceptance_criteria == null)
      throw new Error("azure_work_item_update requires at least one field");
  }
  if (task.type === "codex_dispatch") {
    projectId(task.project); multilineText(task.instruction, "instruction", 4000);
    if (!CODEX_MODES[task.mode]) throw new Error("invalid codex_dispatch mode");
    if (task.timeout_seconds != null && (!Number.isSafeInteger(task.timeout_seconds) || task.timeout_seconds < 30 || task.timeout_seconds > 1800))
...
function azureWorkItem(id, runner = command, organization = azureOrganization()) {
  return JSON.parse(runner("az", ["boards", "work-item", "show", "--id", String(id), "--organization", organization, "--output", "json"]));
}
function azureFields(item) {
  const fields = item.fields || {};
  return { title: fields["System.Title"] || "", description: fields["System.Description"] || "",
    acceptance_criteria: fields["Microsoft.VSTS.Common.AcceptanceCriteria"] || "" };
}
function azureStoredValue(key, value) {
  return key === "title" ? value : value.replace(/\s*<\/(h[1-6]|p|li)>/gi, " </$1>");
}
function azureWorkItemUpdate(task, runner = command, organization = azureOrganization()) {
  const beforeItem = azureWorkItem(task.work_item_id, runner, organization);
  const project = beforeItem.fields?.["System.TeamProject"] || "";
  if (project !== AZURE_PROJECT) throw new Error("work item is outside the configured IoTMart 3.0 project");
  const before = azureFields(beforeItem);
  const requested = Object.fromEntries(Object.entries({ title: task.title, description: task.description,
    acceptance_criteria: task.acceptance_criteria }).filter(([, value]) => value != null)
    .map(([key, value]) => [key, azureStoredValue(key, value)]));
  const canonicalizedFields = Object.keys(requested).filter((key) => requested[key] !== task[key]);
  const noOp = Object.entries(requested).every(([key, value]) => before[key] === value);
  if (!noOp) {
    const args = ["boards", "work-item", "update", "--id", String(task.work_item_id), "--organization", organization, "--output", "json"];
    if (task.title != null) args.push("--title", task.title);
    if (task.description != null) args.push("--description", task.description);
    if (task.acceptance_criteria != null) args.push("--fields", `Microsoft.VSTS.Common.AcceptanceCriteria=${task.acceptance_criteria}`);
    runner("az", args);
  }
  const afterItem = azureWorkItem(task.work_item_id, runner, organization);
  const after = azureFields(afterItem);
  const verified = Object.entries(requested).every(([key, value]) => after[key] === value);
  const result = { work_item_id: task.work_item_id, project, before, after, verified, no_op: noOp,
    ...(canonicalizedFields.length ? { canonicalized_fields: canonicalizedFields } : {}),
    web_url: `${organization}/${encodeURIComponent(project)}/_workitems/edit/${task.work_item_id}`,
    ...(loadMachine() ? { machine_id: loadMachine().id } : {}) };
  return result;
}
function gitStatus(root) { return command("git", ["-C", root, "status", "--porcelain=v1", "--branch"]); }
function dirtyPaths(root) {
  const raw = gitRaw(root, ["status", "--porcelain=v1", "-z"]);
...
function dirtySnapshot(root) {
  const snapshot = {};
  for (const relative of dirtyPaths(root)) {
    const file = path.resolve(root, relative);
    if (!file.startsWith(root + path.sep)) continue;
    const stat = fs.lstatSync(file, { throwIfNoEntry: false });
    snapshot[relative] = !stat ? "missing" : stat.isFile() && !stat.isSymbolicLink()
      ? hash(fs.readFileSync(file)) : `${stat.isDirectory() ? "directory" : "other"}:${stat.size}`;
  }
  return snapshot;
}
function untrackedPaths(root) {
  const raw = gitRaw(root, ["ls-files", "--others", "--exclude-standard", "-z"]);
  return raw ? raw.split("\0").filter(Boolean) : [];
}
function codexFinal(output) {
  let final = "";
  for (const line of String(output || "").split("\n")) {
    try {
      const event = JSON.parse(line);
      if (event.type === "item.completed" && event.item?.type === "agent_message") final = event.item.text || final;
    } catch { /* JSONL is expected, but an error fallback is still useful. */ }
  }
  return clip(final || output, 12000);
}
function codexDispatch(task, options = {}) {
  const root = resolveProject(task.project);
  const before = gitStatus(root), dirtyBefore = dirtySnapshot(root), started = Date.now();
  const prompt = `You are a delegated local Codex executor. Work only in the selected project. Do not commit, push, deploy, change branches, reset, clean, stash, or use external credentials. ${task.instruction}`;
  const run = (options.spawn || spawnSync)("codex", ["exec", "--json", "--sandbox", CODEX_MODES[task.mode], "-C", root,
    "--skip-git-repo-check", prompt], { cwd: root, encoding: "utf8", timeout: (task.timeout_seconds || 900) * 1000, maxBuffer: 2 * 1024 * 1024 });
  const after = gitStatus(root), dirtyAfter = dirtySnapshot(root);
  const preexistingDirtyStateChanged = JSON.stringify(dirtyBefore) !== JSON.stringify(Object.fromEntries(
    Object.keys(dirtyBefore).map((relative) => [relative, dirtyAfter[relative] || "missing"])));
  const readOnlyChanged = task.mode === "read_only" && before !== after;
  const timedOut = run.error?.code === "ETIMEDOUT";
  const exitStatus = Number.isInteger(run.status) ? run.status : null;
  const error = timedOut ? "Codex timed out" : run.error?.message || (exitStatus === 0 ? "" : clip(run.stderr || `Codex exited ${exitStatus}`, 2000));
  const result = { project: task.project, mode: task.mode, machine_id: loadMachine()?.id || "", exit_status: exitStatus,
    elapsed_ms: Date.now() - started, timed_out: timedOut, summary: codexFinal(run.stdout), git_status_before: before,
    git_status_after: after, preexisting_dirty_state_changed: preexistingDirtyStateChanged,
    ...(task.mode === "workspace_write" ? { diff: clip(command("git", ["-C", root, "diff", "HEAD"])), untracked_after: untrackedPaths(root) } : {}),
    ...(error ? { error: clip(error, 2000) } : {}) };
  result.ok = !error && !readOnlyChanged && !preexistingDirtyStateChanged;
  if (readOnlyChanged) result.error = "read-only Codex dispatch changed project state";
  if (preexistingDirtyStateChanged) result.error = "Codex changed pre-existing dirty state";
  return result;
}
...
function runTask(ctx, task) {
  ...
  const ledger = fslog.newTask({ agent: "relay", transport: "github", sandbox: task.type === "project_comment_replace" || task.mode === "workspace_write" ? "workspace-write" : "read-only", cwd: ctx.cwd, prompt });
  ...
  try {
    ...
    } else if (task.type === "azure_work_item_update") {
      result = azureWorkItemUpdate(task);
      if (!result.verified) {
        const error = new Error("Azure work item update verification failed");
        error.result = result;
        throw error;
      }
    } else if (task.type === "codex_dispatch") {
      result = codexDispatch(task);
      if (!result.ok) {
        const error = new Error(result.error || "Codex dispatch failed");
        error.result = result;
        throw error;
      }
    ...
    fslog.writeResult(ledger.dir, error.result ? JSON.stringify(error.result, null, 2) : message);
    fslog.patchStatus(ledger.dir, { status: "error", progress: 100, summary: message });
    return { status: "error", error: message, ...(error.result ? { result: error.result } : {}), ledger_task_id: ledger.id };
  }
}
...
module.exports = { context, putJson, remoteJson, runOnce, watch, runTask, validate, matchesMachine, azureWorkItemUpdate, codexDispatch };
