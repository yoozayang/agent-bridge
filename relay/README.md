...
- `azure_work_item_update` — numeric `work_item_id` and one or more of `title`, `description`, or `acceptance_criteria`. The worker first reads the item, requires it to belong to `IoTMart 3.0`, updates only those supplied fields through Azure CLI, then reads it again. HTML values use Azure DevOps's deterministic close-tag whitespace canonicalization before the exact read-back comparison; no state, assignment, tags, links, attachments, or other metadata can be changed.
- `codex_dispatch` — `project: "iotmart"|"magnolia"`, bounded natural-language `instruction`, `mode: "read_only"|"workspace_write"`, and optional `timeout_seconds` (30–1800). The worker itself invokes `codex exec --json` with only the selected configured root and the fixed matching sandbox. It accepts no CLI flags, paths, commands, environment values, deployment targets, or unrestricted sandbox. Read-only dispatch fails if Git status changes; workspace-write records status/diff and fails if pre-existing dirty files change. It never commits, pushes, changes branches, resets, cleans, stashes, or deploys.
